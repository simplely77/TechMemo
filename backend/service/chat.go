...
func (s *ChatService) buildRAGContext(ctx context.Context, userQuery string, sessionID, userID int64) ([]aiclient.ChatMessage, error) {
	...
	// 混合检索（向量 + 关键词 RRF），与 /search/semantic 前两步一致，提升专有名词、精确短语命中
	topK := InitialTopK
	noteResults, _ := s.searchDao.HybridSearchEmbeddings(ctx, queryVector, userID, userQuery, "note", topK)
	kpResults, _ := s.searchDao.HybridSearchEmbeddings(ctx, queryVector, userID, userQuery, "knowledge", topK)
	...
	for totalTokens > MaxTokens && topK > MinTopK {
		log.Println("totalTokens", totalTokens, "topK", topK)
		topK--
		noteResults, _ = s.searchDao.HybridSearchEmbeddings(ctx, queryVector, userID, userQuery, "note", topK)
		kpResults, _ = s.searchDao.HybridSearchEmbeddings(ctx, queryVector, userID, userQuery, "knowledge", topK)
		contextMsgs = s.mergeContext(ctx, recentMsgs, noteResults, kpResults)
	...
	log.Println("contextMsgs", contextMsgs)
